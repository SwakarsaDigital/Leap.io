...
export default async function ClientDashboard() {
  ...
  // 1. Ambil data Client & Proyek milik mereka dari database
  ...
  // 2. FETCH AVAILABLE FREELANCERS (NEW)
  // Data ini akan dikirim ke LaunchProjectButton agar client bisa assign agent
  const availableFreelancers = await prisma.user.findMany({
    ...
    take: 5 // Batasi 5 freelancer teratas untuk preview
  });
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200">
        <div className="max-w-7xl mx-auto animate-in fade-in duration-1000 pb-20 px-4 md:px-8 py-8">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-6 border-b border-white/5 pb-8">
            ...
            <p className="text-slate-500 text-xs font-mono uppercase tracking-[0.3em] mt-2">
                Command Center // {clientData.company || 'Direct Operation'} // {clientData.name}
            </p>
            </div>
            ...
            {/* Kirim data freelancer ke tombol */}
            <LaunchProjectButton freelancers={availableFreelancers} />
            </div>
        </div>
